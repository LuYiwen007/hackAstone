...
struct ArenaL10n {
    ...
    var agentDisclaimer: String {
        en ? "Views may be generated by an agent; on failure, local text is used." : "观点由 Agent 生成，失败时自动回退本地内容"
    }
    ...
    var debateRoundHint: String {
        en ? "Each round: philosopher reply + judge follow-up; end is decided by the agent."
            : "每轮将由哲学家回应 + 裁判追问；是否结束由 Agent 决定"
    }
    ...
    func philosophyTurnFallback(philosopherName: String, clip: String) -> (reply: String, judge: String) {
        if en {
            let r = "\(philosopherName): Your point on “\(clip)...” is insightful, but we should define key concepts first to avoid shifting premises."
            let j = "Judge: What is the most critical premise in this round? If it fails, how would you revise your view?"
            return (r, j)
        }
        let r = "\(philosopherName)：你的观点「\(clip)...」很有启发，但我们要先界定核心概念，否则容易偷换前提。"
        let j = "Judge：你这轮论证里最关键的前提是什么？如果这个前提不成立，你会如何调整观点？"
        return (r, j)
    }
